...
func (r *RestoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger.V(1).Info("reconciling Restore", "name", restore.Name, "namespace", restore.Namespace)

	if !restore.DeletionTimestamp.IsZero() {
		return ctrl.Result{}, nil
	}

	job, err := findJob(ctx, r.Client, "Restore", restore.Name, restore.Namespace, restore.UID)
	if err != nil {
	...
	if err != nil {
		return r.failRestore(ctx, &restore, err.Error())
	}
	...
	if job == nil {
		...
		if err != nil {
			return r.failRestore(ctx, &restore, fmt.Sprintf("storage location error: %v", err))
		}

		job, err = buildRestoreJob("Restore", restore.Name, restore.Namespace, restore.UID, storage)
		if err != nil {
			return r.failRestore(ctx, &restore, err.Error())
		}
	...
	if job.Status.Failed > 0 && restore.Status.Phase != backupv1alpha1.RestorePhaseFailed {
		return r.failRestore(ctx, &restore, "restore job failed")
	}

	return ctrl.Result{}, nil
}

func (r *RestoreReconciler) getBackupForRestore(ctx context.Context, restore *backupv1alpha1.Restore) (*backupv1alpha1.Backup, error) {
	...
	case "ClusterBackup":
		...
		// Convert to Backup for shared handling.
...
func (r *RestoreReconciler) failRestore(ctx context.Context, restore *backupv1alpha1.Restore, message string) (ctrl.Result, error) {
	now := metav1.Now()
